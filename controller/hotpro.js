...
const addOrUptHots = function(req,res){
	const { brandname,discount,name,starttime,endtime,Hotid} = req.body;
	console.log(req.body)
		if(Hotid){
			...
			if(req.file && req.file.filename){
				saveHots.logoimage=req.file.filename;
			}
			Hots.findByIdAndUpdate(Hotid,{
				$set:saveHots
			}).then((err,ress)=>{
				res.redirect('/hot.html');
			...
			willSaveHots.save().then((err)=>{
				res.redirect('/hot.html');
				res.json({addHotsSuccess:true});
			})	
		}
	}
//前台接口
